...
describe('artist (e2e)', () => {
  ...
  describe('DELETE', () => {
    ...
    it('should set track.artistId to null after deletion', async () => {
      ...
      expect(creationArtistResponse.status).toBe(HttpStatus.CREATED);
      ...
      expect(creationTrackResponse.statusCode).toBe(HttpStatus.CREATED);

      const artistDeletionResponse = await unauthorizedRequest
        .delete(artistsRoutes.delete(artistId))
        .set(commonHeaders);

      expect(artistDeletionResponse.statusCode).toBe(HttpStatus.NO_CONTENT);

      const searchTrackResponse = await unauthorizedRequest
        .get(tracksRoutes.getById(trackId))
        .set(commonHeaders);

      expect(searchTrackResponse.statusCode).toBe(HttpStatus.OK);
    ...
    it('should set album.artistId to null after deletion', async () => {
      ...
      expect(creationAlbumResponse.statusCode).toBe(HttpStatus.CREATED);
      ...
      expect(searchAlbumResponse.statusCode).toBe(HttpStatus.OK);
